...
export const prepare = (state: ProtocolState, broadcast: BroadcastFunction, enterCommitPhase: () => void) => {
  const checkPrepareBallotAcceptQuorum = (ballot: ScpBallot) => {
    ...
    const commitVotes = state.commitStorage
      .getAllValuesAsArary()
      .filter(
        c =>
          hashBallot({ counter: infinityCounter, value: c.ballot.value }) === ballotHash ||
          hashBallot({ counter: c.preparedCounter, value: c.ballot.value }) === ballotHash,
      );
    const externalizeVotes = state.externalizeStorage
      .getAllValuesAsArary()
  ...
  const checkPrepareBallotAcceptBlockingSet = (ballot: ScpBallot) => {
    ...
    const externalizes = state.externalizeStorage
      .getAllValuesAsArary()
      .filter(e => e.commit && hashBallotValue(e.commit) === ballotValueHash);
    const acceptPrepares = state.prepareStorage
      .getAllValuesAsArary()
  ...
  const checkPrepareBallotAcceptCommit = () => {
    ...
    const prepareCommitVotes = state.prepareStorage
      .getAllValuesAsArary()
      .filter(x => hashBallotValue(x.ballot) === ballotValueHash && (x.cCounter <= n && n <= x.hCounter));
    const commits = state.commitStorage
      .getAllValuesAsArary()
  ...
  const checkPrepareBallotConfirm = (ballot: ScpBallot) => {
    ...
    const commits = state.commitStorage
      ...
      .filter(
        c =>
          hashBallot({ counter: c.preparedCounter, value: c.ballot.value }) === ballotHash ||
          hashBallot({ counter: c.hCounter, value: c.ballot.value }) === ballotHash,
      )
      .map(c => c.node);
    const externalizes = state.externalizeStorage
      .getAllValuesAsArary()
  ...
  const updateCommitBallot = () => {
    if (
      (state.commitBallot &&
        isBallotLower(state.commitBallot, state.prepare.prepared!) &&
        hashBallotValue(state.prepare.prepared) !== hashBallotValue(state.commitBallot)) ||
  ...
  const recalculateCCounter = () => {
    updateCommitBallot();
    if (state.commitBallot === null || state.prepare.hCounter === 0) {
      state.prepare.cCounter = 0;
    } else {
      state.prepare.cCounter = state.commitBallot.counter;
    }
  };
